...
export type LoopRecoveryDecisionKind = 'already_completed' | 'requires_human' | 'mark_corrupted' | 'recoverable_auto' | 'requires_legacy_bridge'
...
export class LoopRecovery {
  ...
  async decideLoopRecovery(loopId: string): Promise<LoopRecoveryDecision> {
    ...
    if (state.status === 'completed') return { decision: 'already_completed', reason: 'loop is already completed', loopId, runId: state.lastRunId, warnings: state.warnings }
    if (!state.tasks.length) return { decision: 'mark_corrupted', reason: 'loop has no task events', loopId, runId: state.lastRunId, warnings: state.warnings }
    if (state.pendingHumanGateId) return { decision: 'requires_human', reason: 'loop is waiting for a human gate', loopId, runId: state.lastRunId, warnings: state.warnings }
    if (state.lastRunId) {
      const latestCheckpoint = await this.durable.readLatestCheckpoint(state.lastRunId)
      if (latestCheckpoint?.pendingExternalEffects.some(effect => !effect.confirmed)) return { decision: 'requires_human', reason: 'latest checkpoint has pending external effects', loopId, runId: state.lastRunId, warnings: state.warnings }
      if (latestCheckpoint?.status === 'unsafe_to_replay') return { decision: 'requires_human', reason: 'latest checkpoint is unsafe to replay', loopId, runId: state.lastRunId, warnings: state.warnings }
      const audit = await this.durable.auditRun(state.lastRunId)
      if (!audit.ok || audit.errors.length) return { decision: 'mark_corrupted', reason: audit.errors[0] ?? 'durable audit failed', loopId, runId: state.lastRunId, warnings: [...state.warnings, ...audit.warnings] }
    }
  ...
  async recoverLoop(input: string | { loopId: string; autoRun?: boolean }): Promise<LoopRecoveryResult> {
...
function isV2LoopEvent(event: EventEnvelope): boolean {
