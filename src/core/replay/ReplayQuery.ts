import type { ReplayQuery, ReplayScope } from './ReplayTypes.js'
import { ReplayQueryError } from './ReplayTypes.js'

const SCOPES = new Set<ReplayScope>([
  'run',
  'thread',
  'loop',
  'goal',
...
export function normalizeReplayQuery(input: Partial<ReplayQuery>): ReplayQuery {
  const scope = input.scope
  if (!scope) throw new ReplayQueryError('missing_scope', 'Replay query requires an explicit scope.', input)
  if (!SCOPES.has(scope)) throw new ReplayQueryError('invalid_scope', `Unknown replay scope: ${String(scope)}`, input)
...
export function validateReplayQuery(query: ReplayQuery): void {
  if (query.scope !== 'workspace' && query.scope !== 'time_range') assertScopedId(query)
  if (query.scope === 'workspace') {
    const allowed = query.includeAll && (query.caller === 'test' || query.caller === 'maintenance')
    if (!allowed) throw new ReplayQueryError('unbounded_workspace', 'Workspace replay requires includeAll=true and caller=test|maintenance.', query)
  }
  if (query.scope === 'time_range') {
    ...
    if (!hasSeqRange && !hasTimeRange) throw new ReplayQueryError('invalid_range', 'time_range replay requires seq or time bounds.', query)
  }
...
function applyIdAlias(query: ReplayQuery): void {
  if (!query.id) return
  switch (query.scope) {
    case 'run': query.runId ??= query.id; break
    case 'thread': query.threadId ??= query.id; break
    case 'loop': query.loopId ??= query.id; break
    case 'goal': query.goalId ??= query.id; break
    case 'gui_action': query.guiActionId ??= query.id; break
    case 'gateway_inbound': query.inboundId ??= query.id; break
    case 'gateway_delivery': query.deliveryId ??= query.id; break
    case 'model_route': query.routeId ??= query.id; break
    case 'checkpoint': query.checkpointId ??= query.id; break
    default: break
  }
}

function assertScopedId(query: ReplayQuery): void {
  const ok = Boolean(
    query.runId ||
    query.threadId ||
    query.loopId ||
  ...
  if (!ok) throw new ReplayQueryError('missing_id', `Replay scope ${query.scope} requires an id.`, query)
}
