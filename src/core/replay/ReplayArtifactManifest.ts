...
import type { ReplayArtifactKind, ReplayArtifactManifest, ReplayArtifactRef } from './ReplayTypes.js'

const ARTIFACT_KEYS: Array<{ pattern: RegExp; kind: ReplayArtifactKind }> = [
...
export class ReplayArtifactManifestBuilder {
  build(events: readonly EventEnvelope[]): ReplayArtifactManifest {
    const artifacts: ReplayArtifactRef[] = []
    const warnings: string[] = []
    for (const event of events) collectArtifacts(event.payload, event, '$', artifacts)
...
function collectArtifacts(value: unknown, event: EventEnvelope, path: string, artifacts: ReplayArtifactRef[]): void {
...
function stableArtifactId(eventId: string, key: string, ref: string): string {
