...
import type { ReplayProjection, ReplayProjections } from './ReplayTypes.js'

export function projectAllReplay(events: readonly EventEnvelope[]): ReplayProjections {
...
export function projectRunReplay(events: readonly EventEnvelope[], timeline = new EventReplay().buildTimeline(events)): ReplayProjection {
...
export function projectThreadReplay(events: readonly EventEnvelope[], timeline = new EventReplay().buildTimeline(events)): ReplayProjection {
...
export function projectLoopReplay(events: readonly EventEnvelope[], timeline = new EventReplay().buildTimeline(events)): ReplayProjection {
  const loopEvents = events.filter(event => event.loopId || event.eventType.startsWith('loop_'))
  if (!loopEvents.length) return projection({ timeline: [], status: 'unknown', summary: 'no loop events', importantIds: {}, data: {}, metrics: { loops: 0 } })
...
export function projectGuiReplay(events: readonly EventEnvelope[], timeline = new EventReplay().buildTimeline(events)): ReplayProjection {
...
export function projectGatewayReplay(events: readonly EventEnvelope[], timeline = new EventReplay().buildTimeline(events)): ReplayProjection {
...
export function projectModelReplay(events: readonly EventEnvelope[], timeline = new EventReplay().buildTimeline(events)): ReplayProjection {
...
export function projectCheckpointReplay(events: readonly EventEnvelope[], timeline = new EventReplay().buildTimeline(events)): ReplayProjection {
...
export function projectRecoveryReplay(events: readonly EventEnvelope[], timeline = new EventReplay().buildTimeline(events)): ReplayProjection {
...
export function projectApprovalReplay(events: readonly EventEnvelope[], timeline = new EventReplay().buildTimeline(events)): ReplayProjection {
...
export function projectToolReplay(events: readonly EventEnvelope[], timeline = new EventReplay().buildTimeline(events)): ReplayProjection {
...
function projection(input: { timeline: ReplayTimelineItem[]; status: ReplayProjection['status']; summary: string; importantIds: Record<string, string[]>; data: Record<string, unknown>; metrics: Record<string, number>; warnings?: string[]; errors?: string[] }): ReplayProjection {
...
function statusFromTerminal(eventType: string | undefined, fallback: ReplayProjection['status']): ReplayProjection['status'] {
...
function mapLoopStatus(status: string): ReplayProjection['status'] {
...
function summarizeLatest(events: readonly EventEnvelope[]): Record<string, unknown> | undefined {
...
function modelSelection(event: EventEnvelope): Record<string, unknown> | undefined {
...
function shallowPayload(value: unknown): Record<string, unknown> {
