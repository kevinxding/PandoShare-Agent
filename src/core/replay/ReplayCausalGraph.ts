...
import type { ReplayEdge, ReplayEdgeType, ReplayGraph, ReplayNode, ReplayNodeType } from './ReplayTypes.js'

export class ReplayCausalGraphBuilder {
  build(events: readonly EventEnvelope[]): ReplayGraph {
    ...
    for (const event of sorted) {
      ...
      if (event.parentEventId) {
        ...
        if (nodes.has(parentId)) addEdge(edges, parentId, eventNodeId(event), 'parent_event', false)
        else warnings.push(`event ${event.seq} parentEventId not found: ${event.parentEventId}`)
      }
...
function eventNode(event: EventEnvelope): ReplayNode {
  return {
...
function identityNodes(event: EventEnvelope): ReplayNode[] {
  ...
  if (isLegacy(event)) push(nodes, 'legacy', event.eventId)
  return nodes
}

function addTypedEdges(event: EventEnvelope, edges: ReplayEdge[]): void {
  const payload = recordPayload(event.payload)
  ...
  if (inboundId && commandId) addEdge(edges, `gateway_inbound:${inboundId}`, `command:${commandId}`, 'gateway_to_command', true)
  if (commandId && runId) addEdge(edges, `command:${commandId}`, `run:${runId}`, 'command_to_run', true)
  if (attemptId && runId) addEdge(edges, `attempt:${attemptId}`, `run:${runId}`, 'attempt_to_run', true)
  if (event.loopId && attemptId) addEdge(edges, `loop:${event.loopId}`, `attempt:${attemptId}`, 'loop_to_attempt', true)
  if (runId && routeId) addEdge(edges, `run:${runId}`, `model_route:${routeId}`, 'run_to_model', true)
  if (runId && event.toolCallId) addEdge(edges, `run:${runId}`, `tool_call:${event.toolCallId}`, 'run_to_tool', true)
  if (event.toolCallId && guiActionId) addEdge(edges, `tool_call:${event.toolCallId}`, `gui_action:${guiActionId}`, 'tool_to_gui', true)
  if (guiActionId && checkpointId) addEdge(edges, `gui_action:${guiActionId}`, `checkpoint:${checkpointId}`, 'gui_to_checkpoint', true)
  if (routeId && event.eventType === 'model_usage_recorded') addEdge(edges, `model_route:${routeId}`, eventId, 'model_to_usage', true)
  if (checkpointId && event.eventType.includes('recovery')) addEdge(edges, `checkpoint:${checkpointId}`, eventId, 'checkpoint_to_recovery', true)
  if (approvalId && event.eventType.includes('resolved')) addEdge(edges, `approval:${approvalId}`, eventId, 'approval_to_resolution', true)
  if (routeId && event.eventType === 'model_fallback_selected') addEdge(edges, eventId, `model_route:${routeId}`, 'fallback_to_model', true)
  if (deliveryId && inboundId) addEdge(edges, `gateway_inbound:${inboundId}`, `gateway_outbound:${deliveryId}`, 'gateway_to_command', true)
}

function edgeTypeForNode(type: ReplayNodeType): ReplayEdgeType {
  if (type === 'run') return 'same_run'
  if (type === 'thread') return 'same_thread'
  if (type === 'legacy') return 'legacy_bridge'
  if (type === 'model_route') return 'run_to_model'
  if (type === 'tool_call') return 'run_to_tool'
  if (type === 'checkpoint') return 'gui_to_checkpoint'
  return 'same_run'
}

function upsert(nodes: Map<string, ReplayNode>, node: ReplayNode): void {
  if (!nodes.has(node.nodeId)) nodes.set(node.nodeId, node)
}

function push(nodes: ReplayNode[], type: ReplayNodeType, value: string | undefined): void {
  if (!value) return
  nodes.push({ nodeId: `${type}:${value}`, type, label: `${type} ${value}`, importantId: value })
}

function addEdge(edges: ReplayEdge[], from: string, to: string, type: ReplayEdgeType, synthetic: boolean, reason?: string): void {
  if (from === to) return
  edges.push({ edgeId: `edge_${hash(`${from}:${to}:${type}`)}`, from, to, type, synthetic, reason })
}

function dedupeEdges(edges: readonly ReplayEdge[]): ReplayEdge[] {
  const byId = new Map<string, ReplayEdge>()
  for (const edge of edges) byId.set(edge.edgeId, edge)
  return [...byId.values()]
}

function eventNodeId(event: EventEnvelope): string {
...
function payloadSummary(payload: unknown): unknown {
