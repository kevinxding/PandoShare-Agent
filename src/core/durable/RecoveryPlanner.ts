import type { RecoveryDecision, RecoveryPlannerInput } from './RecoveryDecision.js'

export class RecoveryPlanner {
  decide(input: RecoveryPlannerInput): RecoveryDecision {
...
function decision(runId: string, kind: RecoveryDecision['decision'], reason: string, recoverable = false): RecoveryDecision {
  return {
    decision: kind,
    reason,
    runId,
...
function isAutoRecoverablePhase(phase: string): boolean {
