...
function renderReport({ evidence }) {
  ...
  const lines = [
    ...
    '- Web Gateway serves the integrated product Web UI from client-ui/dist by default while backend APIs stay in src/server/index.ts.',
    ...
    '- Web/API: `src/server/index.ts`; product Web UI source: `client-ui/src/*`; default static output: `client-ui/dist`.',
    ...
    '',
    '- Build: 
pm run client-ui:install once, then 
pm run client-ui:build.',
    '- Serve: `pando serve --static-root <dist>` or `node bin/pando.js serve --static-root <dist>`.',
    '- The server prints the local URL at startup and serves client-ui/dist with the same-origin backend API.',
    '',
    '## 7. Gateway Usage',
    '',
    ...
    '',
    'The old bundled backend Web UI has been removed. Product Web UI work now lives in client-ui/ and is served by Gateway from client-ui/dist by default.',
    '',
  ]
